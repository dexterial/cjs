...
function draw_cEl_text(cEl_ctx,cEl,boolDrawCp){
    ...
    try{ 
        switch(cEl.data.values.pattern){
            case "table":
                //draw_cEl_text_tbl(cEl_ctx,cEl,boolDrawCp);
                return true;
            break;
            case "boxed-horizontal":
                // TODO add custom fonting for chars for example at specific position
                //TODO move this somewhere and create a text path object to be used when filling the chars
                
                //cEl_setPath(cEl_ctx, cEl, cEl.shape.redraw, true);
...
function draw_cEl_text_tbl(cEl_ctx,cEl,boolDrawCp){
...
//        set_cEl_text_widths(cEl_ctx,cEl);
//        
//        //create_text_lines(cEl);
//        
//        //draw_cEl_lines(cEl,cEl_ctx);
//        
//        if(boolDrawCp)cEl_drawCp(cEl.shape,cEl_ctx);
...
function get_text_path(cEl){
    
    try{
//        cdebug(cEl.id,true,true,0);
//        cdebug(cEl.data.values.pattern,false,true,0);
//        cdebug(cEl.data.values.temp.style,false,true,0);
//        cdebug(cEl.data.values.temp.lines3,false,true,0);
...
function draw_cEl_lines_justify(cEl_ctx,cEl,cEl_pageText){
    ...
    try{
        ...
        get_text_path(cEl);
        //cdebug(cEl.id,true,false,0);
        if(cEl.id === "test1"){
            testPointInShape(cEl);
            return true;
        }
        
        // usage of == instead of === is intentional
        var cEl_Selection = cEl_pageText.charsSelection;
        var boolHasSelection = (cEl_Selection.id == cEl.parentId + "_" +cEl.id);
        if(cEl.active && cEl.data.values.editable){
            cEl_Selection.cr.vis = true;
        }else{
            cEl_Selection.cr.vis = false;
        }
...
//        }
        ...
        for(var i = 0, chrObj, charStyle; i< len;i++){  
            ...
            if(i > linePars.bp || chrObj.pp){
                ...
                charY+= linePars.fs*1;
                if(charY> linePars.maxY && i>0)break;
            }else{
                chrObj.nl=false;
            }

            if (cEl.data.values.vertical){
                chrObj.xy = [charY,charX];
            }else{
            ...
            if(chrObj.sc){
                chrObj.w = linePars.sw;
            }
            if( i === linePars.bp && chrObj.sc)chrObj.w = 0;
            
            // reset carret position and style
            if(cEl_Selection.cr.vis && cEl_Selection.cr.pos === i)setCarret(cEl_Selection.cr.vis,chrObj,cEl_Selection.cr.left);
            
            // draw character
            draw_chr(chrObj,i,cEl_ctx,cEl_Selection,boolHasSelection);
